...
export function UniversePage({ universe }: { universe: ServiceUniverse }) {
  ...
  return <>
    ...
    <section className="section universe-categories"><p className="eyebrow">Categorias</p><div className="category-directory">{categories.filter(({ slug }) => slug !== "depilacao").map((category, index) => {
      const categoryId = category.slug === "cabelos" ? "hair-spa" : category.slug === "unhas" ? "manicure" : category.slug === "spa" ? "spa" : category.slug;
      return <CategoryLink category={category.slug} categoryId={categoryId} key={category.slug}><span>0{index + 1}</span><div><h2>{category.name}</h2><p>{category.description}</p></div><b aria-hidden="true">→</b></CategoryLink>;
