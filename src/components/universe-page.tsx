...
import { getCategoriesByUniverse, getServiceCatalogHref, type ServiceUniverse } from "@/data/services";
...
export function UniversePage({ universe }: { universe: ServiceUniverse }) {
  ...
  return <>
    ...
    <section className="section universe-categories"><p className="eyebrow">Categorias</p><div className="category-directory">{categories.map((category, index) => <Link href={getServiceCatalogHref(category.slug)} key={category.slug}><span>0{index + 1}</span><div><h2>{category.name}</h2><p>{category.description}</p></div><b aria-hidden="true">→</b></Link>)}</div><Link className="text-link" href="/servicos">Ver todos os serviços →</Link></section>
