...
export default async function Home() {
  ...
  return <>
    <section className="hero image-hero"><Image src={homeImages.hero} alt="Fachada principal da Lanai no Shopping Downtown" fill priority sizes="100vw" style={{ objectPosition: "center" }} /><div className="image-shade" /><div className="hero-copy"><p className="eyebrow">Estética · Spa · Beleza</p><h1>Experiência<br />Lanai</h1><WhatsAppLink origin="home_hero">Agende seu horário</WhatsAppLink></div><a className="scroll-cue" href="#universos">Descubra ↓</a></section>
    ...
    <section className="instagram"><div className="instagram-heading"><p className="eyebrow">Instagram</p><h2>Conecte-se com a experiência Lanai</h2><a className="text-link" href={OFFICIAL_LINKS.instagram} target="_blank" rel="noreferrer">@lanai_spa →</a></div><div className="instagram-grid">{gallery.map((item) => <a href={item.href} target="_blank" rel="noreferrer" key={item.id}><Image src={item.image} alt={item.alt} fill sizes="(max-width: 800px) 50vw, 25vw" /></a>)}</div></section>
  </>;
}
